define([
  ...
  return declare(null, {
    eventListeners: function(t) {
      t.url =
        "http://services2.coastalresilience.org/arcgis/rest/services/Resilient_Coastal_Cities/Indonesia/MapServer";
      t.obj.visibleLayers = [73];
      t.dynamicLayer = new ArcGISDynamicMapServiceLayer(t.url, {
        opacity: 1 - t.obj.sliderVal / 10
      });
      ...
      function onEventClick(t, evt) {
        ...
        t.map.on("click", function(evt) {
          if (evt.graphic) {
            ...
            $("#" + t.id + "numOfTweetsInAdmin").html(
              t.clicks.numberWithCommas(atts.total)
            );
            // update num of people affcted
            $("#" + t.id + "popWrapperTotal").html(
              t.clicks.numberWithCommas(atts.POP_TOTAL)
            );
            // update people affcetd labels and chart
            $(".rc-menPopNum").html(t.clicks.numberWithCommas(atts.POP_MEN));
            $(".rc-womenPopNum").html(
              t.clicks.numberWithCommas(atts.POP_WOMEN)
            );
            ...
            atts.placeTotal =
              atts.PLACE_WSHP + atts.PLACE_EDU + atts.PLACE_HOSP;
            ...
            $($(".rc-placesText").find("span")[4]).html(
              t.clicks.numberWithCommas(atts.LIVE_RDS_KM) + " km"
            );
            $($(".rc-placesText").find("span")[5]).html(
              t.clicks.numberWithCommas(atts.LIVE_AG_HA) + " ha"
            );
            $($(".rc-placesText").find("span")[6]).html(
              t.clicks.numberWithCommas(atts.LIVE_RICE_HA) + " ha"
            );
            ...
            if (atts.MNG_POTENTIAL > 0 && atts.MNG_RICECONVERT > 0) {
              ...
              $(".rc-manResPotenWrapper h2").html(
                t.clicks.numberWithCommas(atts.MNG_POTENTIAL) + " Hectares"
              );
              $(".rc-ricePotenWrapper h2").html(
                t.clicks.numberWithCommas(atts.MNG_RICECONVERT) + " Hectares"
              );
            }
            $(".rc-lowRicePotenWrapper h2").html(
              t.clicks.numberWithCommas(atts.MNG_LOWRICEPROD) + " Field(s)"
            );
            $(".rc-restMangAndRice h2").html(
              t.clicks.numberWithCommas(atts.mangAndRiceTotal) + " Hectares"
            );
          } else {
        ...
        try {
          // try to clear graphics layer if there are graphics pushed into it
          t.obj.layer.clear();
        } catch (err) {
          ("catch for the try");
        }
        ...
        if (evt.currentTarget.id === t.id + "dateRangeGo") {
          ...
          $.each(t.obj.eventList, function(i, v) {
            ...
            if (
              t.obj.daterangeStartCustom < newEnd &&
              t.obj.daterangeEndCustom > newStart
            ) {
          ...
          t.obj.changeDate(
            t.obj.daterangeStartCustom,
            t.obj.daterangeEndCustom
          );
        } else {
          getTagsFromEvents(
            evt.currentTarget.id,
            evt.currentTarget.dataset.date.split(" - ")[0],
            evt.currentTarget.dataset.date.split(" - ")[1]
          );
        }
        function buildGraphic(geom, atts, total) {
          ...
          var adapSym = new SimpleFillSymbol(
            SimpleFillSymbol.STYLE_SOLID,
            new SimpleLineSymbol(
              SimpleLineSymbol.STYLE_SOLID,
              new Color([0, 0, 0]),
        ...
        function populateData() {
          // update num of tweets in event
          $("#" + t.id + "numOfTweetsInAdmin").html(
            t.clicks.numberWithCommas(t.tagTotal)
          );
          // update num of people affcted
          $("#" + t.id + "popWrapperTotal").html(
            t.clicks.numberWithCommas(t.popTotal)
          );
          // update people affcetd labels and chart
          ...
          $($(".rc-placesText").find("span")[4]).html(
            t.clicks.numberWithCommas(t.roadsTotal) + " km"
          );
          $($(".rc-placesText").find("span")[5]).html(
            t.clicks.numberWithCommas(t.agTotal) + " ha"
          );
          $($(".rc-placesText").find("span")[6]).html(
            t.clicks.numberWithCommas(t.riceTotal) + " ha"
          );

          // update adaptation solutions text
          $(".rc-manResPotenWrapper h2").html(
            t.clicks.numberWithCommas(t.mangTotal) + " Hectares"
          );
          $(".rc-ricePotenWrapper h2").html(
            t.clicks.numberWithCommas(t.convRiceTotal) + " Hectares"
          );
          $(".rc-lowRicePotenWrapper h2").html(
            t.clicks.numberWithCommas(t.lowRiceTotal) + " Field(s)"
          );
          $(".rc-restMangAndRice h2").html(
            t.clicks.numberWithCommas(t.mangAndRiceTotal) + " Hectares"
          );
        }
      ...
      function showEventButtons(val) {
        ...
        dateReformat30 =
          dateReformat30[1] + "/" + dateReformat30[2] + "/" + dateReformat30[0];
      ...
      function buildClickEvents() {
        ...
        $("#" + t.id + "rangeToggle input").on("click", function(evt) {
          ...
          var wrappers = $(".rc-floodTimeframeWrapper").find(
            ".rc-rangeWrapper"
          );
          ...
          $(".rc-eventsWrapperInner .rc-event")
            .off()
        ...
        $(".rc-infraCBWrapper input")
          ...
          .on("click", function(evt) {
            ...
            if (checked) {
              ...
              // if checked == false remove layer from viz layers array
        ...
        $(".rc-headingAndBackBtnWrapper button")
          .off()
          .on("click", function(evt) {
            // clear map graphics
    ...
    appSetup: function(t) {
      ...
      function getEvents() {
        ...
        t.obj.getDate = function(d, time) {
          ...
          if (newDay >= 0) {
            var newDate = String(
              Date.fromDayofYear(newDay).toLocaleDateString()
            );
            ...
            var newYear =
              newDate.substring(newDate.length - 1, newDate.length) - 1;
        ...
        function getFloodEvents(endDate) {
          var url =
            "https://api.floodtags.com/v1/events/index?until=" +
            todaysDate +
            "&since=" +
            endDate +
            "&upperLimit=10000&filterSource=northern-java&apiKey=e0692cae-eb63-4160-8850-52be0d7ef7fe";
          t.obj.eventList = [];
        ...
        function createAdminUnitList() {
          // filter the tags object to make sure we do not display event
          // buttons that have no matching tags *********
          let adminUnitList = [];
          $.each(t.obj.adminUnits, (i, v) => {
            let id = v.attributes.geonameid.split(":")[1];
            id = id.substring(0, id.length - 1);
            if (id.includes(",")) {
              id = id.split(",");
              $.each(id, (i, v) => {
                id = "g-" + v;
                adminUnitList.push(id);
              });
            } else {
              id = "g-" + id;
              adminUnitList.push(id);
        ...
        function checkTagsInAdminList(data) {
          let featuresNum = data.features.length;
          let numOfNoMatch = 0;
          $.each(data.features, (i, feat) => {
            let allGeoNameId = feat.properties.geonameid;
            if (t.obj.adminUnitList.indexOf(allGeoNameId) <= 0) {
              numOfNoMatch += 1;
            }
          });
          if (numOfNoMatch === featuresNum) {
            return false;
          } else {
            return true;
          }
        ...
        function filterEvents(array) {
          t.obj.adminUnitList = createAdminUnitList();
          ...
          $.each(array, function(i, v) {
            ...
            var url =
              "https://api.floodtags.com/v1/tags/northern-java/geojson.json?since=" +
              startDate +
              "T00:00:00.000Z&until=" +
              endDate +
              "T23:59:59.000Z&parentGeonameid=g-1627893&apiKey=e0692cae-eb63-4160-8850-52be0d7ef7fe";
            $.get(url, function(data) {
              var defer = $.Deferred(),
                filtered = defer.then(function() {
                  let tagAdminMatch = checkTagsInAdminList(data);
        ...
        function buildEventButtons(array) {
          ...
          $.each(array, function(i, v) {
            ...
            var html =
              ...
              date +
              "' class='rc-event' id='" +
              v.location.geonameid +
              "'><span class='floodEventText'>Flood event: </span><div class='rc-floodEventDate'>  " +
              v.start.split("T")[0] +
              " - " +
              v.end.split("T")[0] +
              "</div></div>";
